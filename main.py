...
async def get_user_info(rawQuery, authToken, ct0, cursor = None, product = 'Latest'):
    client = Client("en-US")
    client.set_cookies({
...
@app.get("/Following")
async def Following(authToken, ct0, restId, cursor = None, count = 20):
    client = Client("en-US")
    client.set_cookies({
        "auth_token": authToken,
        "ct0": ct0,
    })
    response, _ = await client.gql.following(
        user_id = restId,
        cursor = cursor,
        count = count
    )
